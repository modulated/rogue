use super::{BuilderChain, BuilderMap, InitialMapBuilder, AreaStartingPosition, DistantExit, random_start_position, TileType};
...
impl TownBuilder {
	...
	fn build_rooms(&mut self, rng: &mut Rng, build_data : &mut BuilderMap) {
	...
	fn water_and_piers(&mut self, rng: &mut Rng, build_data : &mut BuilderMap) {
	...
	fn town_walls(&mut self, rng: &mut Rng, build_data : &mut BuilderMap) -> (HashSet<usize>, i32) {
		...
		for y in 1 .. build_data.height - 2 {
			if !(y > wall_gap_y - 4 && y < wall_gap_y + 4) {
				let idx = build_data.map.xy_idx(X_OFFSET, y);
				...
				for x in X_OFFSET .. build_data.width {
		...
		for x in X_OFFSET .. build_data.width - 1 {
	...
	fn buildings(&mut self, rng: &mut Rng, build_data : &mut BuilderMap, available_building_tiles: &mut HashSet<usize>) -> Vec<(i32, i32, i32, i32)> {
	...
	fn add_doors(&mut self, rng: &mut Rng, build_data : &mut BuilderMap, buildings: &mut Vec<(i32, i32, i32, i32)>, wall_gap_y: i32) -> Vec<usize> {
...
pub fn town_builder(new_depth: i32, rng: &mut Rng, width: i32, height: i32) -> BuilderChain {
	let mut chain = BuilderChain::new(new_depth, width, height);
	chain.start_with(TownBuilder::new());
	chain.with(super::VoronoiSpawning::new());
	chain
}
