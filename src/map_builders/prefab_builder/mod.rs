use super::{MetaMapBuilder, InitialMapBuilder, BuilderMap, TileType, Position};
...
impl PrefabBuilder {
	...
	fn apply_previous_iteration<F>(&mut self, mut filter: F, _rng: &mut Rng, build_data : &mut BuilderMap)
	...
	#[allow(dead_code)]
	fn apply_sectional(&mut self, section : &prefab_sections::PrefabSection, rng: &mut Rng, build_data : &mut BuilderMap) {
		use prefab_sections::*;
	...
	fn apply_room_vaults(&mut self, rng : &mut Rng, build_data : &mut BuilderMap) {
		use prefab_rooms::*;
		...
		let master_vault_list = vec![TRAP_ROOM, CHECKERBOARD, SILLY_SMILE];
