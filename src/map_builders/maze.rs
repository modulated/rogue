use super::{MapBuilder, Map,  
	TileType, Position, super::spawner, SHOW_MAPGEN_VISUALIZER,
...
impl<'a> Grid<'a> {
	...
	fn get_available_neightbors(&self) -> Vec<usize> {
	...
	fn find_next_cell(&mut self) -> Option<usize> {
		let neighbors = self.get_available_neightbors();
		...
		loop {
			...
			match next {
				Some(next) => {
					...
					let (lower_part, higher_part) = self.cells.split_at_mut(max(self.current, next));
