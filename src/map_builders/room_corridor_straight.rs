...
pub struct StraightCorridors {}

impl MetaMapBuilder for StraightCorridors {
	#[allow(dead_code)]
...
impl StraightCorridors {
	#[allow(dead_code)]
	pub fn new() -> Box<Self> {
		Box::new(Self{})
	}

	fn corridors(&mut self, _rng: &mut Rng, build_data : &mut BuilderMap) {
		let rooms: Vec<Rect>;
		if let Some(rooms_builder) = &build_data.rooms {
			rooms = rooms_builder.clone();
		} else {
			panic!("StraightCorridor requires builder with room structures.");
		}
		...
		for (i, room) in rooms.iter().enumerate() {
			...
			if !room_distance.is_empty() {
				...
				let line = rltk::line2d(rltk::LineAlg::Vector, room_center_pt, rltk::Point::new(dest_center.0, dest_center.1));
